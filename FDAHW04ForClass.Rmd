...
(C) Code the loop to have 200 repetitions -- that is the for loop has the indexing/structure 
``` for( k in 1:200)
{
#   analyze  each sample, Y[,k]
}
```
...
fitObjGCV<- Tps(s,Y[,k])
```
In the second case  lambda (and the corresponding effective degrees of freedom)  is being found by the  GCV criterion. 
...
( for example ``` meanO3<- rowMeans(O3)```)
